...
def handle_status(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        total = len(entries)
        downloaded = sum(1 for e in entries if e.status == DownloadStatus.DOWNLOADED)
        failed = sum(1 for e in entries if e.status == DownloadStatus.FAILED)
        skipped = sum(1 for e in entries if e.status == DownloadStatus.SKIPPED_DUPLICATE)
        
        total_duration = sum(e.duration_seconds for e in entries if e.status == DownloadStatus.DOWNLOADED)
        total_duration_hours = total_duration / 3600
        
        # Print statistics
        print(f"\n{'='*60}")
        print(f"GraphHansard Miner — Download Statistics")
        print(f"{'='*60}")
        print(f"\nTotal entries:        {total}")
